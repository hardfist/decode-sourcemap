...
async function finishLoadingCodeWithEmbeddedSourceMap(code, file) {
  ...
  if (url) {
    ...
    showLoadingError(
      `Failed to find an embedded "/${c}# sourceMappingURL=" comment in the ${
        file ? "imported file" : "pasted text"
      }.`
    );
  }
}
...
function decodeMappings(mappings, sourcesCount, namesCount) {
  ...
  function decodeVLQ() {
    ...
    while (true) {
      ...
      if ((c & 0x7f) !== c)
        decodeError(
          `Invalid mapping character: ${JSON.stringify(String.fromCharCode(c))}`
        );
